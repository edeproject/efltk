...
//        DEVMODE mode;
        ...
        info.depth = 8;//mode.dmBitsPerPel;
...
wchar_t *fl_utf2locale(const char *s, UINT codepage = 0)
{
	if (!s) return L"";
	int len, l = 0;
	len = strlen(s);
...
{
	wchar_t buf[8];
	buf[GetLocaleInfo(id, LOCALE_IDEFAULTANSICODEPAGE, buf, 8*sizeof(wchar_t))] = 0;
...
void fl_get_codepage()
{
	...
	DWORD ccp = wcstol(ld,'\0',10); //atol(ld);
	fl_is_ime = false;
        ...
        default:                 // avoid compiler warning
            ...
            if (!_TrackMouseEvent)
            {
                ...
                if (handle) _TrackMouseEvent =
                        (BOOL(WINAPI*)(_TRACKMOUSEEVENT*))GetProcAddress(handle, L"TrackMouseEvent");
