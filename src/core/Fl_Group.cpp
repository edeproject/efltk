...
void Fl_Group::ctor_init() {
    widget_type(GROUP_TYPE);
    style(::group_style);
    align(FL_ALIGN_TOP);

    m_auto_grow = FL_GROUP_NO_GROW;
    m_auto_grow_min_width = 0;
    m_auto_grow_min_height = 0;
        ...
        while (a < e)
        {
            ...
            switch (o->layout_align()) {
                case 0: {
                        ...
                        if (m_auto_grow_min_width < R) {
                            m_auto_grow_min_width = R;
                            if (m_auto_grow_min_width > w())
                                extend_w = m_auto_grow_min_width - w();
                        }

                        if (m_auto_grow_min_height < B) {
                            m_auto_grow_min_height = B;
                            if (m_auto_grow_min_height > h())
                                extend_h = m_auto_grow_min_height - h();
                        }
                    }
                    break;
            ...
            if (o->layout_align()) {
                if (pref_x+pref_w > w())
                    extend_w = pref_x+pref_w - w();

                if (pref_y+pref_h > h())
                    extend_h = pref_y+pref_h - h();
                /*
                printf("%s: Preferred size w,h = %ix%i\n",o->label().c_str(),pref_w,pref_h);
                printf("%s: Extend         w,h = %ix%i\n",o->label().c_str(),extend_w,extend_h);
                puts("");
                */
            }

            if (extend_w)
                extend_w = max( w() / 5, extend_w + offset * 2 ); // grow by necessary extend, or +20%
