...
static boolean fill_input_buffer (j_decompress_ptr cinfo)
{
    ...
    nbytes = jpeg_io.read(src->buffer, INPUT_BUFFER_SIZE);
    if (nbytes <= 0) {
...
static bool jpeg_create(uint8 *&data, Fl_PixelFormat &fmt, int &w, int &h)
{
...
static bool jpeg_read_file(FILE *fp, int quality, uint8 *&data, Fl_PixelFormat &format, int &w, int &h)
{
	...
	return jpeg_create(data, format, w, h);
}
