...
static const Fl_Menu_Item* add(Fl_Group* g, const Fl_Menu_Item* m,void* data)
{
    ...
    while (m && m->text)
    {
        ...
        if(m->flags&FL_MENU_RIGHTLAYOUT) ((Fl_Menu_Bar *)g)->right_layout(o);

        // these flags have been cleverly assigned so this shift and mask
        // converts from the old values to the new ones:
        o->set_flag((m->flags<<12)&(FL_INACTIVE|FL_VALUE|FL_INVISIBLE));
...
const Fl_Menu_Item* Fl_Menu_Item::pulldown(int X, int Y, int W, int H,
                                           ...
                                           const char* title) const
{
    ...
    if (menu.Fl_Menu_::popup(X, Y, W, H))
