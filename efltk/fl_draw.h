...
FL_API void fl_mult_matrix(float a, float b, float c, float d, float e, float f);
...
inline int 	fl_clip_box(int _x, int _y, int _w, int _h, int& x, 
		int& y, int& w, int& h)			   
		{return fl_current_dev->clip_box(_x, _y, _w, _h, x, y, w, h);};
...
//fl_curve.cpp
inline void fl_curve(float a,float b,float c,float d,float e,float f,float g,float h)
	{fl_current_dev->curve(a,b,c,d,e,f,g,h);}
//fl_arc.cpp
inline void fl_arc(float x,float y,float w,float h, float a1, float a2)
	{fl_current_dev->arc(x, y, w,h, a1,a2);}
//fl_vertex.cpp
inline void fl_circle(float x, float y, float r)
	{fl_current_dev->circle(x, y, r);}
inline void fl_ellipse(float x, float y, float w, float h)
	{fl_current_dev->ellipse(x, y, w, h);}
...
inline void fl_pie(int x,int y,int w,int h,float a1,float a2,int what=FL_PIE) 
	{fl_current_dev->pie(x,y,w,h,a1,a2,what);};
