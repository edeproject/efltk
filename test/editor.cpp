...
void scrolling_cb(Fl_Widget*, void* v) {
   EditorWindow* e = (EditorWindow*)v;
   load_file("editor_test1.txt",-1);
   Fl_Date_Time	start = Fl_Date_Time::Now();
   unsigned i = 0;
   for (; e->editor->move_down(20); i+=20) {
      e->editor->scroll(i,0);
      Fl::check();
   }
   char msg[128];
   sprintf(msg,"Scrolled %i lines for %0.2f seconds",i,((double)Fl_Date_Time::Now() - double(start))*3600*24);
   fl_alert(msg);
}
...
Fl_Menu_Item menuitems[] = {
   ...
   { "&Tests", 0, 0, 0, FL_SUBMENU },
   { "&Scrolling",       FL_CTRL + '1', scrolling_cb },
   { 0 },
