...
//not good example, but descriptive one
void print(Fl_Widget *, void *w) 
{
	Fl_Widget * g = (Fl_Widget *)w;
//	char * filename = fl_file_chooser("Postscript file", "*.ps", "*.ps");
//	if(!filename) return;
	FILE * f = fopen("image1.ps", "w");
	if(!f) return;
	Fl_Printer * p = new Fl_PostScript(f,3,Fl_Printer::A4,Fl_Printer::PORTRAIT);
	p->page();
	p->fit(g, FL_ALIGN_TOP);
	p->draw(g);
	delete p;
};
...
// Make RGB image
Fl_Image rgb_image(WIDTH, HEIGHT, 24, make_image(), true, 0x0000FF, 0x00FF00, 0xFF0000, 0);
