...
void show_alignment_cb(Fl_Widget *, void *) 
{
	...
	*/
	horizontal_input->value(gridx);
	vertical_input->value(gridy);
	snap_input->value(snap);
	...
	if ((damage()&FL_DAMAGE_ALL) &&
		...
		for	(int y = 0; y < h(); y += CHECKSIZE) {
			for (int x = 0; x < w(); x += CHECKSIZE) {
				fl_color(((y/(2*CHECKSIZE))&1) != ((x/(2*CHECKSIZE))&1) ? FL_WHITE : FL_BLACK);
...
int Fl_Window_Type::handle(int event) 
{
  ...
  case FL_RELEASE:
		...
		if (drag != BOX && (dx || dy || !Fl::event_is_click())) {			
			((Fl_Group*)o)->init_sizes(); //Allow resize even if we have resizable..
			...
			if (i) {
				...
				for (;;) {
					if (backtab) {
						...
						i = i->walk(this);
					if (!i) break;
					if (i->is_widget() && !i->is_menu_item() && ((Fl_Widget_Type*)i)->o->parent()->visible_r()) 
						break;
					}
				}
				if (!i) i = first_child;
...
void Fl_Window_Type::read_property(const Fl_String &c) 
{
	if(c=="modal") {
		...
		if (Fl::first_window()) 
			open(); // only if we are using user interface
	} else if(c=="noborder") {
		border = 0;
	} else if (c=="xclass") {
