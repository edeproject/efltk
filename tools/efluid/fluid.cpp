...
"The EFLTK user interface designer version 2.0.3\n"
"Copyright 2002-2003 by EDE team.\n"
...
"USA.\n"
"\n"
"Please report bugs to Laza@Flashmail.com";
...
#ifdef WIN32
#include <efltk/win32.h>	// for MAX_PATH definition
#endif
...
char *lc(double v) {
    ...
    char decimal = '.';
#if ENABLE_NLS
    if(!locale_conv) locale_conv = localeconv();
...
char *lc2(const char *v) {
    static char ret[128];
    strncpy(ret, v, sizeof(ret));
...
void goto_source_dir() {
  ...
  const char *p = fl_file_filename(filename);
  ...
  if (*buffer && chdir(buffer)<0) {fprintf(stderr, "Can't chdir to %s : %s\n",
...
void save_cb(Fl_Widget *, void *v) {
  ...
  if (v || !c || !*c) {
    if (!(c=fl_save_file(c, "Fluid files, *.f[ld]", "Save to:"))) return;
    set_filename(c);
  }
  if (!write_file(c)) {
...
void open_cb(Fl_Widget *, void *v) {
  ...
  const char *c;
  if (!(c = fl_select_file(filename, "Fluid files, *.{fl|fld}", "Open:"))) return;
  if (!v) set_filename(c);
  if (!read_file(c, v!=0)) {
...
void new_cb(Fl_Widget *, void *v) {
  if (!v && modflag && !fl_ask("Discard changes?")) return;
  const char *c;
  if (!(c = fl_save_file(0, "Fluid files, *.{fl|fld}", "New:"))) return;
  delete_all();
  set_filename(c);
  modflag = 0;
}
...
void write_cb(Fl_Widget *, void *) {
  ...
  char cname[1024];
  char hname[1024];
  strcpy(i18n_app, fl_file_filename(filename));
  fl_file_setext(i18n_app, "");

  if (*code_file_name == '.') {
    strcpy(cname,fl_file_filename(filename));
    fl_file_setext(cname, code_file_name);
  } else {
    strcpy(cname, code_file_name);
  }
  if (*header_file_name == '.') {
    strcpy(hname,fl_file_filename(filename));
    fl_file_setext(hname, header_file_name);
  } else {
...
void write_strings_cb(Fl_Widget *, void *) {
    ...
    char sname[1024];
    strcpy(sname, fl_file_filename(filename));
    fl_file_setext(sname, ".po");
    if (!compile_only) goto_source_dir();
...
static char* cutfname() {
  ...
  if (!beenhere) {beenhere = 1; fl_file_expand(name,256,name);}
  return name;
#endif
}
...
{"&Help",0,0,0,FL_SUBMENU|FL_MENU_RIGHTLAYOUT},
...
void make_main_window() {
  if (!main_window) {
    ...
    Fl_Pack *pack = new Fl_Pack(0,0,WINWIDTH,WINHEIGHT);
    pack->begin();
...
"%s\n", argv[0], Fl::help);
  ...
  if (!compile_only)
  {
      Fl_Image::state_effect_all(false);
